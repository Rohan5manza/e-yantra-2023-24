...
def classify_event(image):
    ...
    model.fc = nn.Linear(model.fc.in_features, len([combat, rehab, military_vehicles, fire, destroyed_building]))
    model.load_state_dict(torch.load("C:/Users/rohan/Desktop/e-yantra 2023 Robotics (Geoguide)/task 2B/fine_tuned_model.pth"))
    model.eval()
    model.to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))

    # Define the data transformation for images
    data_transform = transforms.Compose([
        transforms.Resize(size=(64, 64)),
        transforms.ToTensor()])
    try:
        with Image.open(image) as img:
            transformed_image = data_transform(img).unsqueeze(0).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
            with torch.no_grad():
                outputs = model(transformed_image)
                _, predicted = torch.max(outputs, 1)

            # Map the predicted class index to the event name
            class_names = [combat, rehab, military_vehicles, fire, destroyed_building]
            event = class_names[predicted.item()]
            return event
    except Exception as e:
        print("Error classifying event:", e)
        return "unknown"
